...
const NAV_BY_ROLE = {
  ...
  dealer: [
    { path: '/dealer-dashboard',        label: 'Dashboard',       icon: 'grid' },
    { path: '/dealer/inventory',        label: 'Inventory',       icon: 'box' },
    { path: '/dealer/manage-listings',  label: 'Manage Listings', icon: 'list' },
    { path: '/dealer/rentals',          label: 'Rentals',         icon: 'key' },
    { path: '/dealer/test-drives',      label: 'Test Drives',     icon: 'car' },
    { path: '/dealer/sales',            label: 'Sales',           icon: 'trending' },
    { path: '/dealer/messages',         label: 'Messages',        icon: 'message' },
    { path: '/dealer/profile',          label: 'Profile',         icon: 'user' },
  ],
  service_provider: [
    { path: '/service-provider-dashboard',    label: 'Dashboard',    icon: 'grid' },
    { path: '/service-provider/appointments', label: 'Appointments', icon: 'calendar' },
    { path: '/service-provider/services',     label: 'Services',     icon: 'tool' },
    { path: '/service-provider/availability', label: 'Availability', icon: 'clock' },
    { path: '/service-provider/messages',     label: 'Messages',     icon: 'message' },
    { path: '/service-provider/profile',      label: 'Profile',      icon: 'user' },
  ],
  admin: [
    { path: '/admin-dashboard',         label: 'Dashboard',      icon: 'grid' },
    { path: '/admin/users',             label: 'Users',          icon: 'users' },
    { path: '/admin/dealers',           label: 'Dealers',        icon: 'building' },
...
const Icon = ({ name }) => {
  const icons = {
    ...
    car:       <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 17H3a2 2 0 0 1-2-2V9a2 2 0 0 1 2-2h1l2-4h10l2 4h1a2 2 0 0 1 2 2v6a2 2 0 0 1-2 2h-2"/><circle cx="7.5" cy="17.5" r="2.5"/><circle cx="16.5" cy="17.5" r="2.5"/></svg>,
    ...
    building:  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/></svg>,
    ...
    <aside className={`dashboard-sidebar${collapsed ? ' sidebar-collapsed' : ''}`}>
      ...
      <div className="sidebar-content">
        <nav className="sidebar-nav">
          {/* Home link — always visible so users can return to the public site */}
          <Link
            to="/"
